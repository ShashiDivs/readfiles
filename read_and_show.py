...
def read_excel(file_path: str, log) -> str:
    ...
    log.info("Excel read complete", extra={"file": Path(file_path).name, "rows": len(rows)})
    return "\n".join(rows)


def extract(raw_text: str, file_type: str, log) -> dict:
    log.info("Sending to GPT-4o for extraction", extra={"file_type": file_type})
    start = time.time()
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user",   "content": f"Raw Excel data:\n\n{raw_text}\n\nReturn valid JSON only."}
        ],
        temperature=0,
...
def run():
    ...
    raw_bank = read_excel(str(bank_file), log_extract)
    bank_data = extract(raw_bank, "bank_statement", log_extract)
    ...
    raw_gl = read_excel(str(gl_file), log_extract)
    gl_data = extract(raw_gl, "bank_gl", log_extract)
